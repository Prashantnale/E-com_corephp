...
<div class="mb-3">
  <label for="formGroupExampleInput" class="form-label">categaroy Name</label>
...
if($name!="" && $brand!="" && $folder!="" && $id!="")
{
    ...
    if(!$tname)
    {
        $query="UPDATE `category` SET `name`='$name',`categaroy`='$brand' WHERE `id`='$id'";
    }
    else
    {
        $query="UPDATE `category` SET `name`='$name',`categaroy`='$brand',`img`='$folder' WHERE `id`='$id'";
    }
